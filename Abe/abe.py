...
<body>
    <h1><a href=\"%(dotdot)s%(HOMEPAGE)s\"><img
     src=\"%(dotdot)s%(STATIC_PATH)slogo32.png\" class=\"logoimg\" style=\"vertical-align: middle;\" alt=\"DarkCoin logo\" /></a> %(h1)s
    </h1>
    %(body)s
    <p style=\"font-size: smaller\">
        <span style=\"font-style: italic\">
            DarkCoin Explorer powered by <a href=\"%(ABE_URL)s\">%(APPNAME)s</a>
        </span><br />
        Tips appreciated!
...
class Abe:
    ...
    def handle_chain(abe, page):
        ...
        nav = ['<a class="btn-sm" href="',
               basename, '?count=', str(count), '">&lt;&lt;</a>']
        nav += [' <a class="btn-sm" href="', basename, '?hi=', str(hi + count),
                 '&amp;count=', str(count), '">&lt;</a>']
        nav += [' ', '&gt;']
        if hi >= count:
            nav[-1] = ['<a class="btn-sm" href="', basename, '?hi=', str(hi - count),
        ...
        #extra = True
        body += ['<p>', nav, '</p>\n',
                 '<table class="table-bordered"><tr><th>unicode("区块高度")</th><th>unicode("区块时间")</th>',
                 '<th>unicode("交易数")</th><th>unicode("交易额")</th>',
                 '<th>unicode("难度")</th><th>unicode("已开采量")</th>',
                 '<th>unicode("平均年龄")</th><th>unicode("链年龄")</th>',
                 '<th>% ',
                 '<a href="https://en.bitcoin.it/wiki/Bitcoin_Days_Destroyed">',
                 'CoinDD</a></th>',
